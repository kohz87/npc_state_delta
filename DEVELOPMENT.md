...
Delta 1.0.10 is a focused recovery/persistence hotfix on the released 1.0.9 baseline. It retains unsaved state on every normal persistence failure exit, preserves referenced journal versions during same-message consolidation, checkpoints assistant receipts before scans, and exempts terminal-death dossiers from stale cleanup regardless of archive status. The 256 raw-message horizon, separate byte-bounded full checkpoints, supported v1.0.7 readers, Speech ledger and scoring remain intact. Scanner prompts, routes, requests/retries/output allowances and persisted formats are unchanged. See `docs/v1.0.10-review.md` for findings, actual checks and limitations; earlier reports remain historical evidence.
