...
Delta 1.0.8 is a recovery/persistence hardening release on the completed nine-stage 1.0.0 line. It retains the accepted 1.0.1 through 1.0.7 behavior while repairing the rollback/recovery boundary without changing scanner prompts or numerical mechanics. Recovery now classifies same lineage, forward extension, tail truncation and replacement divergence separately, so a stale persisted lineage that is merely an exact prefix of the live chat never restores an older checkpoint. The reversible journal uses a 256 raw-message horizon from a trustworthy baseline, crosses unchanged user/system boundaries, coalesces repeated same-message commits and stores social-graph changes as edge/slot deltas instead of whole-graph copies. A permanently rejected durable write keeps the newest undurable canonical snapshot in the persistence owner so bounded chat-cache eviction cannot reload an older sidecar during the same session; stale lifecycle deletion removes owned structured references while leaving portrait bytes available until rollback history no longer reaches the removed NPC. Active sidecar payloads use compact JSON/base64 construction. Scanner routes, retries, output allowances, relationship formulas, bundle schema, sidecar format version and branch-lineage version remain unchanged. Stage 9 evidence remains in `docs/stage9-review.md`; immutable seed records and earlier stage reports retain their historical results.
