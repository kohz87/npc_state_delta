...
Delta 1.0.4 is a bounded post-release targeted-Refresh parity hotfix on the completed nine-stage 1.0.0 line. It retains the accepted 1.0.1 apparent-age, 1.0.2 birthday, and 1.0.3 implicit-form/native-export fixes, and changes only the direct per-NPC Refresh prompt adapter so it concretely advertises the active Stage 4 appearance-form fields, carries the target's established form context, and recognizes natural anatomical transitions without adding another model request. Scanner routing, retries, relationship mechanics, output allowance, persistence ownership, bundle schema and automatic scan behavior remain unchanged. Stage 9 evidence remains in `docs/stage9-review.md`; immutable seed records and earlier stage reports retain their historical results.
