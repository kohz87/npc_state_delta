...
Delta 1.0.9 is a narrow persistence follow-up to the completed 1.0.8 recovery hardening release. It retains the accepted lineage classifier, 256 raw-message rollback horizon, unchanged-boundary traversal, same-message coalescing, social-graph delta undo, stale-reference cleanup, portrait retention and compact sidecar encoding from 1.0.8. When a permanent sidecar rejection leaves a storage-owned undurable snapshot and bounded cache eviction later forces runtime hydration, the hydration path now preserves that undurable status instead of advancing the in-memory persisted-version marker. The recovered canonical state therefore remains pending until a later successful flush actually reaches disk. Scanner routes, retries, output allowances, relationship formulas, bundle schema, sidecar format version and branch-lineage version remain unchanged. Stage 9 evidence remains in `docs/stage9-review.md`; immutable seed records and earlier stage reports retain their historical results.
