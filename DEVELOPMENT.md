...
Delta 1.0.13 is a focused exact-boundary recovery hotfix on the released 1.0.12 baseline. Delete/edit may restore only the requested surviving parent from the rollback journal or an exact checkpoint; a retained older ancestor can never substitute for a missing parent. Near-tail replacement divergence can now use journal rollback even after its full checkpoint was pruned. Deep edits with multiple retained assistant descendants fail closed and preserve current canonical dossiers because the routine scanner cannot deterministically replay the retained suffix. Exact known swipe siblings still restore without another request; unseen siblings may restore only their exact parent/root anchor and then rescan. The 1.0.12 one-time compaction, 8 MB aggregate / 2 MB per-snapshot checkpoint limits, 256 raw-message journal horizon and 12 MB diagnostic target remain unchanged. Scanner prompts, routes, requests/retries/output allowances, relationship scoring, storage schema and bundle format are unchanged.
