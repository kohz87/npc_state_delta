...
Delta 1.0.12 is a focused branch-retention hardening candidate on the released 1.0.11 baseline. Existing sidecars receive one proof-gated legacy branch-history compaction: active lineage and SillyTavern-retained swipe alternatives survive, while unreachable old sibling checkpoints/cards and unowned rollback-journal chains are removed only after lineage is safe; unresolved destructive divergence defers cleanup until reconciliation. v4-to-v5 migration now maps only host-proven active/swipe keys. The aggregate full-checkpoint budget is 8 MB with a 2 MB per-snapshot ceiling; the separate 256 raw-message journal contract and 12 MB diagnostic target are unchanged. The 1.0.11 linear delete/regenerate, fail-closed recovery and Important-Bonds hardening remain intact. Scanner prompts, routes, requests/retries/output allowances, relationship scoring, storage schema and bundle format are unchanged.
