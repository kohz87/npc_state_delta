...
Delta 1.0.14 is a focused Important-Bonds idempotence/deletion hotfix on the released 1.0.13 baseline. Legacy bond dynamics containing repeated structural pipes, spaced slash fragments, tiny truncation debris or repeated relation labels are normalized deterministically so repeated reconciliation cannot grow the entry. Manual deletion removes the shared social edge and the counterpart's mirrored structured bond in the same transaction, including when the visible owner entry is already malformed but its subject prefix still resolves. New later story/scanner evidence may establish the relationship again normally. Delta 1.0.13 exact-boundary recovery, the 1.0.12 one-time compaction, 8 MB aggregate / 2 MB per-snapshot checkpoint limits, 256 raw-message journal horizon and 12 MB diagnostic target remain unchanged. Scanner prompts, routes, requests/retries/output allowances, relationship scoring, storage schema and bundle format are unchanged.
