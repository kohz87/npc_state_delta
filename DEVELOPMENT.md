...
Delta 1.0.5 is a bounded post-release manual-bond reconciliation hotfix on the completed nine-stage 1.0.0 line. It retains the accepted 1.0.1 apparent-age, 1.0.2 birthday, 1.0.3 implicit-form/native-export and 1.0.4 targeted-Refresh fixes. The runtime change is confined to the existing `social.js` owner: a manual Key Relationships / Important Bond edit now replaces the edited NPC's graph direction instead of enriching a stale same-counterpart edge, while preserving a compatible reverse-side bond for the counterpart. It also normalizes recognized `relation; dynamic` input and removes a redundant orphan line already contained in a structured bond. Scanner prompts, routes, retries, relationship scoring formulas, output allowances, persistence formats, bundle schema and automatic scan behavior remain unchanged. Stage 9 evidence remains in `docs/stage9-review.md`; immutable seed records and earlier stage reports retain their historical results.
