...
Delta 1.0.6 is a bounded post-release speech-development hotfix on the completed nine-stage 1.0.0 line. It retains the accepted 1.0.1 apparent-age, 1.0.2 birthday, 1.0.3 implicit-form/native-export, 1.0.4 targeted-Refresh, and 1.0.5 manual-bond fixes. The runtime change remains inside the existing `core.js` facade over the accepted durable-profile engine: Speech gets a bounded deterministic development ledger keyed by stable concept plus turn/source-message provenance, so repeated independent observations can authorize gradual evolution without counting replay of one message. Existing grounded `explicit` and `batch` evolution remains authoritative and resets the old speech evidence epoch when accepted; a bare time skip does not change Speech or clear pending evidence. Scanner prompts, routes, retries, output allowances, relationship formulas, persistence format versions, bundle schema and automatic scan behavior remain unchanged. Stage 9 evidence remains in `docs/stage9-review.md`; immutable seed records and earlier stage reports retain their historical results.
