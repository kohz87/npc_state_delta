...
const APPEARANCE_RULES = `\nSTAGE 4 APPEARANCE: appearance is the current visible presentation, not only permanent anatomy. Capture directly described hair/body traits, current outfit/gear, and relevant visible condition. If newer narration reveals an earlier visual impression was wrong or obscured, refine to the corrected FULL current appearance; latest explicit visual evidence wins. If the presentation itself changes, use change+reason and return the FULL current appearance. overallAppearance is only form-independent visual detail. Named anatomical presentations use appearanceForms:[{name,appearance,state:"refine|change",reason}] plus currentForm/currentFormState:"select". A switch preserves other forms. If transformation is visible but its stable form name is unknown, use currentFormState:"unknown" and grounded current appearance only; never reuse another form's anatomy. Omission preserves forms/selection. Locked Appearance protects overall/current presentation, forms and selection.`;
