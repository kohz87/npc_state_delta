...
8. ROLE/SPECIES/GENDER/BACKGROUND may update when this window establishes or clarifies them. Gender is only "male" or "female" and only from explicit wording or an unambiguous gendered identity/reference; never infer it from name, role, clothing, body type, species, or portrait. Once established, preserve it unless stronger explicit evidence requires genderState:"correct"+genderReason. HOME BASE / USUAL LOCATION is durable home, workplace, headquarters, or regular haunt, never the temporary current scene location; establish only from grounded ongoing-life evidence, and changing an established value requires homeBaseState:"update"+homeBaseReason. Species is literal only. Background is durable history, not current mood/status.
...
11. Age/ApparentAge separate: age=chronology only; apparentAge=visual cue, compact ~N, never prose; species literal; no species-aging inference. gender="male"|"female" only when explicit wording or an unambiguous gendered identity/reference establishes it; never infer from name, role, clothing, body type, species, or portrait. Once established, omit/keep unless stronger explicit evidence requires genderState:"correct"+genderReason. Birthday/exact elapsed=>ageState:"advance"+reason; correction=>ageState:"correct"+reason; visual aging/growth/rejuvenation=>apparentAgeState:"evolve"+reason. Appearance must not repeat explicit age. Vague time skip insufficient.
