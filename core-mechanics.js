...
function groundedAppearanceChange(existing, incoming, reason, context = '', evidenceItems = [], binding = null) {
    ...
    // A declared current-presentation change replaces obsolete presentation rather than
    // refining it clause-by-clause. The replacement itself must still be source-grounded.
    return durableSeedGrounded(incoming, support);
}
...
7. DURABLE PROFILE CHANNEL: ALWAYS emit one top-level profileUpdates item even without npc delta. One scene may support multiple fields; emit each grounded (speech+behaviorProfile allowed). Appearance=CURRENT VISIBLE PRESENTATION: hair/body, outfit/gear, condition. EXISTING NPC + explicit new/corrected/current visual evidence => MUST emit appearance plus evidence.appearance in profileUpdates; never omit it because other profile fields are also returned. Correction/reveal=>appearanceState:"refine"+FULL Appearance; changed clothes/form/presentation=>appearanceState:"change"+reason+FULL Appearance. matching *State:"refine"=>FULL; personality/speech/mannerism "evolve"+reason. behaviorProfile FULL max6; Mannerisms FULL max4. behavior->relationshipSummary/live/Memory. lockedProfileFields never rewrite.
