...
7. DURABLE PROFILE CHANNEL: ALWAYS emit one top-level profileUpdates item for durable facts even without npc delta. No duplicate/inferred. One scene may support multiple fields; emit each grounded item independently (speech+behaviorProfile allowed). matching *State:"refine" returns FULL field; lasting personality/speech/mannerism "evolve"+reason; appearance "change"+reason. behaviorProfile FULL max6 target-general response/decision levers, not action summaries; actions are evidence for levers. Soft labels may include Disposition, Care/Warmth, Expressiveness, Independence/Agency, Conflict/Assertiveness, Threat Sensitivity, Analytical Style, Social Presentation; do not fill unsupported labels. Mannerisms FULL max4 DISTINCT. PC/one-scene behavior -> relationshipSummary/live state/Memory. lockedProfileFields never rewrite.
