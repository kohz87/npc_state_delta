...
test('Delta application metadata is v1.0.65 and active core ownership is semantic', () => {
    assert.match(core, /NPC_STATE_VERSION = '1\.0\.65'/);
    assert.match(core, /export \* from '\.\/core-mechanics\.js'/);
    assert.doesNotMatch(core, /NPC_STATE_SOURCE_ENGINE_VERSION|core-v0218/);
    assert.equal(manifest.version, '1.0.65');
