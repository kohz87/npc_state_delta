...
Retain legacy sidecar storage, owner-qualified chat keys, lineage/swipe-aware snapshots, writer locking, revision conflicts, dirty-write retention/retry, recovery copies, deletion tombstones, rename/delete lifecycle handling and branch recovery. Full branch checkpoints remain byte-bounded recovery/swipe anchors. Delta 1.0.7 also retains a bounded reversible rollback journal of compact canonical-state deltas keyed by exact branch lineage/sequence so recent deep tail deletion does not depend on a full checkpoint surviving the snapshot byte budget. Journal rollback must reverse structural state as well as fields: an NPC introduced only in deleted history disappears together with owned social references and pending work, while surviving NPC state returns to its prior narrative values. Existing user-owned metadata overlay/tombstone rules remain authoritative and cannot resurrect a narrative NPC whose existence was rolled away. The journal contains no transcript text and performs no model reconstruction. A numeric message position alone is not durable ownership. Preserve active work through transient write failure; never advertise a failed save as durable.
