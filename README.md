...
Stages 1-9 are implemented. Delta 1.0.1 is the first post-release continuity hotfix: it preserves the accepted 1.0.0 architecture and request budgets while making grounded visual-age capture reliable on first encounter. Live SillyTavern/Gemini extraction quality, provider latency, physical-device file pickers and actual image-provider behavior are separate verification boundaries, not claims made by this release. See [Stage 9 evidence](docs/stage9-review.md).
...
Use SillyTavern 1.18.0 or a compatible later host. Install this repository through the host extension installer, or unpack `npc_state_delta-1.0.1.zip` so one `npc_state_delta` folder contains `manifest.json` directly. Reload the host after updating. Keep a native Delta backup before changing an existing installation.
