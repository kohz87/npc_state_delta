...
Stages 1-9 are implemented. Delta 1.0.2 is a bounded post-release hotfix on the accepted 1.0.x architecture: it retains the grounded first-pass apparent-age fix from 1.0.1 and adds manual birthday correction to Edit Dossier through the canonical calendar/birthday engine without adding a model request, scanner pass, retry, or prompt change. Live SillyTavern/Gemini extraction quality, provider latency, physical-device file pickers and actual image-provider behavior are separate verification boundaries, not claims made by this release. See [Stage 9 evidence](docs/stage9-review.md).
...
Use SillyTavern 1.18.0 or a compatible later host. Install this repository through the host extension installer, or unpack `npc_state_delta-1.0.2.zip` so one `npc_state_delta` folder contains `manifest.json` directly. Reload the host after updating. Keep a native Delta backup before changing an existing installation.
