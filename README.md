...
Stages 1-9 are implemented. Delta 1.0.5 is a bounded manual-bond reconciliation hotfix on the accepted 1.0.x architecture. It retains the 1.0.1 grounded apparent-age fix, 1.0.2 manual birthday correction, 1.0.3 implicit anatomical-transition/native-export fixes, and 1.0.4 targeted Refresh form parity. This release fixes manual Important Bond / Key Relationship edits so stale hidden social-graph wording cannot be merged back over the user's corrected bond. No scanner prompt, model request count, retry budget, numerical relationship formula, persistence format or bundle schema is changed. Live SillyTavern/Gemini extraction quality, provider latency, physical-device file pickers and actual image-provider behavior are separate verification boundaries, not claims made by this release. See [Stage 9 evidence](docs/stage9-review.md).
...
Use SillyTavern 1.18.0 or a compatible later host. Install this repository through the host extension installer, or unpack `npc_state_delta-1.0.5.zip` so one `npc_state_delta` folder contains `manifest.json` directly. Reload the host after updating. Keep a native Delta backup before changing an existing installation.
